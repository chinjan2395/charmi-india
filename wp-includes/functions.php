...
function misha_brand_column( $columns_array ) {
	return array_slice( $columns_array, 0, 3, true )
	       ...
	       + array( 'polish' => 'Pol' )
	       + array( 'sym' => 'Sym' )
	       + array( 'fluorescence-color' => 'Fluor' )
//	       + array( 'fluorescence-intensity' => 'Fluorescence Intensity' )
	       + array( 'shade' => 'Shade' )
	       + array( 'luster' => 'Luster' )
	       + array( 'ec' => 'EC' )
	       + array( 'lab' => 'Lab' )
	       ...
	       + array_slice( $columns_array, 0, NULL, true );
}
...
function misha_populate_brands( $column_name ) {
	...
	switch ( $column_name ) {
		case 'availability':
			$meta = isset_column( $meta_data[0], 'availability' );
			echo $meta;
			break;
		case 'shape':
			$meta = isset_column( $meta_data[0], 'shape' );
			echo $meta;
			break;
		case 'weight':
			$meta = isset_column( $meta_data[0], 'weight' );
			echo $meta;
			break;
		case 'color':
			$meta = isset_column( $meta_data[0], 'color' );
			echo $meta;
			break;
		case 'clarity':
			$meta = isset_column( $meta_data[0], 'clarity' );
			echo $meta;
			break;
		case 'cut':
			$meta = '';
			if ( isset( $meta_data[0]['cut'] ) || isset( $meta_data[0]['pa_cut'] ) ) {
				$meta = $meta_data[0][ isset( $meta_data[0]['cut'] ) ? 'cut' : 'pa_cut' ]['value'];
			}
			echo $meta;
			break;
		case 'polish':
			$meta = isset_column( $meta_data[0], 'polish' );
			echo $meta;
			break;
		case 'sym':
			$meta = isset_column( $meta_data[0], 'sym' );
			echo $meta;
			break;
		case 'fluorescence-color':
			$meta = isset_column( $meta_data[0], 'fluorescence-color' );
			echo $meta;
			break;
		case 'fluorescence-intensity':
			$meta = isset_column( $meta_data[0], 'fluorescence-intensity' );
			echo $meta;
			break;
		case 'shade':
			$meta = isset_column( $meta_data[0], 'shade' );
			echo $meta;
			break;
		case 'luster':
			$meta = isset_column( $meta_data[0], 'luster' );
			echo $meta;
			break;
		case 'ec':
			$meta = isset_column( $meta_data[0], 'ec' );
			echo $meta;
			break;
		case 'lab':
			$meta = isset_column( $meta_data[0], 'lab' );
			echo $meta;
			break;
		case 'table':
			$meta = isset_column( $meta_data[0], 'table' );
			echo $meta;
			break;
		case 'depth':
			$meta = isset_column( $meta_data[0], 'depth' );
			echo $meta;
			break;
		case 'ratio':
			$meta = isset_column( $meta_data[0], 'ratio' );
			echo $meta;
			break;
		case 'measurements':
			$meta = isset_column( $meta_data[0], 'measurements' );
			echo $meta;
			break;
	}

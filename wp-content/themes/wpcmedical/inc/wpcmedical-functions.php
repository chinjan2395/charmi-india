<?php
/**
 * WPCmedical functions.
 *
 * @package wpcmedical
 */

if ( ! function_exists( 'wpcmedical_is_woocommerce_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function wpcmedical_is_woocommerce_activated() {
 ...
 *
 */
function wpcmedical_do_shortcode( $tag, array $atts = array(), $content = null ) {
 ...
 * Accounts for the WPCmedical Designer and WPCmedical Powerpack content background option.
 *
 * @return string the background color
 * @since  1.6.0
 */
function wpcmedical_get_content_background_color() {
	if ( class_exists( 'WPCmedical_Designer' ) ) {
		$content_bg_color = get_theme_mod( 'sd_content_background_color' );
		$content_frame    = get_theme_mod( 'sd_fixed_width' );
	}

	if ( class_exists( 'WPCmedical_Powerpack' ) ) {
	...
	if ( class_exists( 'WPCmedical_Powerpack' ) || class_exists( 'WPCmedical_Designer' ) ) {
...
/**
 * Apply inline style to the WPCmedical header.
 *
 * @uses  get_header_image()
 * @since  2.0.0
 */
function wpcmedical_header_styles() {
	...
	$styles = apply_filters( 'wpcmedical_header_styles', $styles );
...
/**
 * Apply inline style to the WPCmedical homepage content.
 *
 * @uses  get_the_post_thumbnail_url()
 * @since  2.2.0
 */
function wpcmedical_homepage_content_styles() {
	...
	$styles = apply_filters( 'wpcmedical_homepage_content_styles', $styles );
 ...
 * @since  1.0.0
 */
function wpcmedical_adjust_color_brightness( $hex, $steps, $opacity = 1 ) {
 ...
 * @since  1.3.0
 */
function wpcmedical_sanitize_choices( $input, $setting ) {
...
add_filter( 'the_content_more_link', 'wpcmedical_continue_reading_link' );
 ...
 */
function wpcmedical_customizer_custom_css() {
	?>
    <style type="text/css">
        <?php
			if ( get_theme_mod( 'wpcmedical_page_title_background_image' ) ) {
				$page_title_background_image_url = get_theme_mod( 'wpcmedical_page_title_background_image' );
			} else {
				$page_title_background_image_url = '';
			}
		?>

        .page-title-bar {
            background-image: url( <?php echo esc_url($page_title_background_image_url); ?> );
        }
    </style>
	<?php
} // end wpcmedical_customizer_custom_css

add_action( 'wp_head', 'wpcmedical_customizer_custom_css' );